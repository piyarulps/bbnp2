...
@Component({
  selector: 'app-foliobaseotp',
  templateUrl: './foliobaseotp.component.html',
  styleUrl: './foliobaseotp.component.scss'
})
export class FoliobaseotpComponent implements OnInit {
 ...
 close() {
  ...
  submit(){
    const Data={ "PAN":localStorage.getItem('pancard')}
    this.authService.panBasedOtpGenerate(Data).subscribe(
      res => {
        this.start(1);
        if(res['data'].RefNo){
